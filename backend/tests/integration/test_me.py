...
async def test_get_me_returns_full_user(client: AsyncClient) -> None:
    ...
    expected_keys = {
        ...
        "year_of_birth", "grade_level", "phone_number", "avatar_url",
        "display_locale", "theme_preference", "text_size_preference",
        "reduce_motion", "notif_email_enabled", "notif_inapp_enabled",
        "interests", "role", "status", "created_at",
    }
...
async def test_put_interests_rejects_empty(client: AsyncClient) -> None:
    _user, headers = await signup_and_login(client)
    resp = await client.put(
        "/me/interests", headers=headers, json={"interest_ids": []}
    )
    assert resp.status_code == 422
...
async def test_delete_me_soft_deletes_and_blocks_future_calls(client: AsyncClient) -> None:
    ...
    resp = await client.request(
        "DELETE", "/me", headers=headers, json={"confirm": True}
    )
...
async def test_delete_me_requires_confirm_true(client: AsyncClient) -> None:
    _user, headers = await signup_and_login(client)
    resp = await client.request(
        "DELETE", "/me", headers=headers, json={"confirm": False}
    )
    assert resp.status_code == 422
