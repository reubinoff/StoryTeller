...
@pytest.mark.asyncio
async def test_writing_submit_kicks_eval_and_completes(client: AsyncClient) -> None:
    _user, headers = await signup_and_login(client)
    ...
    # BackgroundTask runs after the response is written. httpx waits for that.
...
async def test_writing_result_while_processing_returns_null_evaluation(
    ...
    # Suppress the BackgroundTask so the task stays in 'processing'.
    import app.api.v1.routers.tasks as tasks_router

    async def _noop(_task_id):
        return None

    monkeypatch.setattr(tasks_router, "run_writing_evaluation", _noop)
...
@pytest.mark.asyncio
async def test_retry_after_failure_triggers_re_evaluation(client: AsyncClient) -> None:
    from app.services import evaluation_service

    fail_count = {"n": 0}

    async def _flaky_eval(task_id):
        fail_count["n"] += 1
        if fail_count["n"] == 1:
            # First call: simulate failure
            from app.db.session import get_sessionmaker
            from app.db.models.task import Task as TaskModel
            from app.db.models._helpers import utcnow as _now

            sm = get_sessionmaker()
            async with sm() as db:
                t = await db.get(TaskModel, task_id)
                if t is None:
                    return
                t.status = "failed"
                t.failed_at = _now()
        ...
        await evaluation_service.run_writing_evaluation(task_id)
    ...
    # Patch ONLY for this submit + retry cycle.
    import app.api.v1.routers.tasks as tasks_router

    monkey_orig = tasks_router.run_writing_evaluation
    tasks_router.run_writing_evaluation = _flaky_eval  # type: ignore[assignment]
    try:
        await client.post(
            f"/tasks/{task_id}/submit",
            headers=headers,
        ...
        tasks_router.run_writing_evaluation = monkey_orig  # type: ignore[assignment]
