...
class TaskAnswer(Base):
    ...
    __table_args__ = (UniqueConstraint("task_id", "question_id", name="uq_task_answers_question"),)

    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid7)
    task_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("tasks.id", ondelete="CASCADE"), nullable=False, index=True
    )
    question_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("task_questions.id", ondelete="CASCADE"))
